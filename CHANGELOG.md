...
### Unreleased
- **Cookies для Instagram**: поддержка файла cookies для yt-dlp (переменная `YTDLP_COOKIES_PATH`, монтирование в контейнер `http-service`). При заданном пути yt-dlp вызывается с `--cookies`, что снимает ограничения «login required» для Instagram. Поддержка формата JSON (экспорт из браузера): файл в формате JSON автоматически конвертируется в Netscape перед вызовом yt-dlp. Папка `cookies/` добавлена в `.gitignore`.
