...
### 0.8.0 – 2026-03-27 – Job-лог, `GET /job-log`, админ `/logs`

- **Пакет `internal/joblog`**: чтение **хвоста** NDJSON-файла без загрузки всего файла в память (**`TailEntries(path, maxLines)`** → **`TailResult`**: **`Entries`** с **`ParsedLine.Fields map[string]any`**, **`ParseErrors`** для битых строк, **`Truncated`** если запрошено больше **`MaxLinesLimit` (100)**). Только стандартная библиотека; юнит-тесты в **`internal/joblog`**. См. **`ARCHITECTURE.md`**.
- **`GET /job-log`** (http-service): хвост NDJSON job-лога (`entries`, `truncated`, `parse_errors`), параметр **`lines`**, чтение через **`internal/joblog`**. Регистрация маршрута только при непустых **`ADMIN_JOB_LOG_TOKEN`** и пути **`JOB_LOG_PATH`** (по умолчанию `/logs/send.log`). Авторизация: **`Authorization: Bearer`** или **`X-Admin-Token`**. Коды **401** / **404** / **503**. Переменные: **`README.md`**, **`.env.example`**, **`docker-compose.yml`**, **`ENV_RESTORE_GUIDE.md`**.
- **Job-логи `/logs/send.log`**: формат **NDJSON** (`log/slog`, `JSONHandler`). В контексте **`POST /send`** — **`request_id`**; **`video_pipeline`** (тайминги **`probe_ms`**, **`ytdlp_ms`**, **`ffmpeg_ms`**); доставка — **`event=delivery`**, **`channel`**, **`stage`**/**`status`**. Обрезка длинных **`url`** — **`logutil.TruncateString`**. Пакеты **`internal/requestid`**, **`internal/logutil`**.
- **Бот: админ-команда `/logs [N]`** — **`GetJobLogPreview`**: запрос к http-service с **`ADMIN_JOB_LOG_TOKEN`**, группировка по **`request_id`**, суффикс **`jobLogUserSuffix`** (**`uid=`**, **`@username`**, **`tg=`**) из полей NDJSON; при длине текста больше ~4096 символов — документ **`job-log.txt`**. Меню админа и **`/help`**. Документация: **`README.md`**, **`ARCHITECTURE.md`**, **`Download_Track.md`**.
- **Cookies: согласованный расчёт дней и надёжность уведомлений**: **`DaysLeftCeil(now, expiry)`** в `internal/downloader`; **`GET /cookie-status`** и админ-уведомления; **`NotifyAdmin` → `bool`**; **`httpserver.AdminNotifier`**. Фоновая проверка: первый прогон при старте; уведомление об истёкших cookies; **`formatDaysRu`**; ретраи (до 3 попыток). Документация: **`README.md`**, **`ARCHITECTURE.md`**.
- **Уведомление админу о cookies Instagram**: пакет **`internal/adminnotify`**, **`CookieExpiry`**, **`GET /cookie-status`**, админ-команда **`/cookie`**.
- **Пайплайн видео**: **`VideoMeta`** и длительности этапов (мс) в **`video_pipeline`**; **`Fetcher`** → **`FetchResult`** с опциональными **`VideoMeta`**.
- **Instagram: формат загрузки** в yt-dlp — **`best`** (один лучший файл).
- **Автотесты**: **`internal/downloader`**, **`internal/adminnotify`**, **`internal/joblog`**, **`internal/httpserver`** (**`GET /job-log`**: без токена маршрут не регистрируется, **401**, **200** с Bearer и **`X-Admin-Token`**, **404** если файла лога нет), сборка **`cmd/bot`** и **`cmd/http-service`**. Список — **`README.md`**, **`ARCHITECTURE.md`**.
